...
# @cocotb.test()
# async def gen1_normal_test(dut):
#     """Testing scrambler with specific input pattern."""
#     global lfsr
#     lfsr = 0xFFFF  # Initialize the LFSR
...
#     dut.rst_i.value = 1
#     dut.scramble_enable_i.value = 1
#     dut.training_sequence_i.value = 0
#     dut.datak_i.value = 0b0001
#     dut.data_len_i.value = 0b00
#     dut.indata_i.value = 0x000000BC
...
#     input_patterns = [
#         {"indata_i": 0x000000BC, "datak_i": 0b0001, "data_len_i": 0b10, "scramble_enable_i": 1, "training_sequence_i": 0},
#         {"indata_i": 0x001C1C1C, "datak_i": 0b0111, "data_len_i": 0b10, "scramble_enable_i": 1, "training_sequence_i": 0},
#         {"indata_i": 0x00000000, "datak_i": 0b0000, "data_len_i": 0b00, "scramble_enable_i": 1, "training_sequence_i": 0},
#         {"indata_i": 0x0000BC00, "datak_i": 0b0010, "data_len_i": 0b10, "scramble_enable_i": 1, "training_sequence_i": 0},
#         {"indata_i": 0x00000000, "datak_i": 0b0000, "data_len_i": 0b01, "scramble_enable_i": 1, "training_sequence_i": 0},
#     ]
...
#         dut.indata_i.value = pattern["indata_i"]
#         dut.datak_i.value = pattern["datak_i"]
#         dut.data_len_i.value = pattern["data_len_i"]
#         dut.scramble_enable_i.value = pattern["scramble_enable_i"]
#         dut.training_sequence_i.value = pattern["training_sequence_i"]
#         data_len = await get_length_from_data_len(pattern["data_len_i"])
#         # Wait for one clock cycle
#         await RisingEdge(dut.clk_i)

#         # Compute the expected scrambled value
#         indata = pattern["indata_i"]
#         expected_scrambled = 0
#         for byte_index in range(4):  # Process 4 bytes from the 32-bit input
#             byte = (indata >> (8 * (3 - byte_index))) & 0xFF
#             scrambled_byte = await scramble_byte(byte)
#             expected_scrambled = (expected_scrambled << 8) | scrambled_byte

#         # Get the DUT scrambled output
#         dut_scrambled_output = dut.scrambled_data_o.value.integer

#         # Check the DUT output matches the Python model
#         if dut_scrambled_output != expected_scrambled:
#             didPass = False
#             dut._log.error(f"Mismatch at pattern: {pattern}")
#             dut._log.error(f"Expected scrambled: 0x{expected_scrambled:08X}, DUT scrambled: 0x{dut_scrambled_output:08X}")

#     assert didPass, "Test failed for one or more input patterns"
# @cocotb.test()
# async def gen1_normal_test(dut):
#     """Testing scrambler with specific input pattern."""
#     global lfsr
...
#         # Scramble the data
#         expected_scrambled_bytes = []
#         indata = pattern["indata_i"]
#         for i in range(data_len):
#             byte_to_scramble = (indata >> (8 * i)) & 0xFF
#             scrambled_byte = await scramble_byte(byte_to_scramble)
#             expected_scrambled_bytes.append(scrambled_byte)

#         # Compare the scrambled bytes
#         scrambled_output = dut.scrambled_data_gen1.value
#         for i in range(data_len):
#             if i == 0:  # Compare with the lowest byte
#                 scrambled_byte_dut = scrambled_output & 0xFF
#             elif i == 1:  # Compare with the second lowest byte
#                 scrambled_byte_dut = (scrambled_output >> 8) & 0xFF
#             elif i == 2:  # Compare with the third lowest byte
#                 scrambled_byte_dut = (scrambled_output >> 16) & 0xFF
#             elif i == 3:  # Compare with the highest byte
#                 scrambled_byte_dut = (scrambled_output >> 24) & 0xFF

#             if scrambled_byte_dut != expected_scrambled_bytes[i]:
#                 didPass = False
#                 dut._log.error(
#                     f"Pattern failed: In Byte[{i}] Expected: 0x{expected_scrambled_bytes[i]:02X}, "
#                     f"Received: 0x{scrambled_byte_dut:02X}"
#                 )

#     assert didPass, "Scrambler failed for one or more patterns."
